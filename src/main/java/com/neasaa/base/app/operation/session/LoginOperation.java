...
public class LoginOperation extends AbstractOperation<LoginRequest, LoginResponse> {
  ...
  public LoginResponse doExecute(LoginRequest opRequest) throws OperationException {
    ...
    ValidationUtils.addToDoLog(
        "Add hostAppName, OS Name and BrowserName during session creation",
        LoginOperation.class.getName());
    AppSession appSession =
        sessionService.createSession(
            authenticatedUser,
            AUTHENTICATED_SESSION,
            ACTIVE_SESSION,
            clientInformation.getChannel(),
            context.getAppHostName(),
            clientInformation.getClientUserIpAddr(),
    ...
    AppSessionUser appSessionUser =
        AppSessionUser.buildAuthenticatedUser(authenticatedUser, roleIdsForUser, appSession);
