...
public abstract class AbstractOperation<
  ...
  @Getter private OperationContext context;
  ...
  public Response execute(Request opRequest, OperationContext context) throws OperationException {
    ...
    if (context == null) {
      log.info(
          "Operation context is not set, set context on operation before calling execute method.");
    ...
    try {
      ...
      log.info(
          "Internal unhandle exception in executing the operation. Error:{}", th.getMessage(), th);
  ...
  protected OperationEntity getOperationEntityByName(String aOperationName)
  ...
  protected boolean isOperationAllowedForUser(String aOperationName) {
    ...
    if (this.context == null || this.context.getAppSessionUser() == null) {
      log.info(
          "Operation context or AppSessionUser is null, cannot check operation allowed for user");
      return false;
    }
    try {
      return this.authorizationService.isOperationAllowedForUser(
          operationEntity, this.context.getAppSessionUser());
