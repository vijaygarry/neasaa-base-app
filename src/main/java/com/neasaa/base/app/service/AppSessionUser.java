...
/**
 * Session user is created and stored in HTTP Session wrapper to share the session information
 * during the session. Login Operation returns new AppSessionUser which then used by session
 * controller to put in HTTP Session object.
 */
@Getter
@Builder
public class AppSessionUser {
  ...
  public static AppSessionUser buildAuthenticatedUser(
      AppUser appUser, List<String> roleIdsForUser, AppSession appSession) {
    if (appUser == null) {
      return null;
    }
