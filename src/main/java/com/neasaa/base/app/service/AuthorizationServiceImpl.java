...
public class AuthorizationServiceImpl implements AuthorizationService {
  ...
  @Override
  public boolean isOperationAllowedForUser(
      OperationEntity aOperationEntity, AppSessionUser appSessionUser) throws OperationException {
    ...
    switch (aOperationEntity.getAuthorizationType()) {
      ...
      case ROLE_BASE:
        ...
        boolean operationAllowed =
            checkOperationAllowedForUserAndChannel(appSessionUser, aOperationEntity);
      ...
      default:
        String msg =
            "Authtype "
                + aOperationEntity.getAuthorizationType()
                + " not supported by operation. Check "
                + aOperationEntity.getOperationId()
                + " configuration.";
        log.info(msg);
        throw ExceptionUtils.getInternalException(
            "Failed to process your request, contact administrator", new Exception(msg));
    }
  }

  private boolean checkOperationAllowedForUserAndChannel(
      AppSessionUser appSessionUser, OperationEntity aOperationEntity) {
