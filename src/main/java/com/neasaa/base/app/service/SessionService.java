...
public class SessionService {
  ...
  @Autowired private AppSessionDao sessionDao;

  @Autowired private AppSessionTxnDao appSessionTxnDao;

  public AppSession createSession(
      AppUser authenticatedUser,
      boolean aAuthenticated,
      boolean sessionActive,
      ChannelEnum aChannel,
      String aAppHostName,
      String aClientIpAddress,
      String aClientOsName,
      String aClientBrowserName)
  ...
  public void checkSessionValidity(AppSessionUser appSessionUser) {
    ...
    if (appSession.getLastAccessTime().getTime() + SESSION_TIMEOUT_MILLIS < currentTime) {
      log.info(
          "Session expired. Session last access time: {}, Current time: {}",
          appSession.getLastAccessTime(),
          currentTime);
      throw new UnauthorizedException("Session expired, please login to perform this operation.");
    }
  }
  ...
  /**
   * Update last access time for this session in separate database transaction. This is to make
   * sure, time is updated even if operation fails.
   */
  @Transactional(
  ...
  public void auditTransaction(
      ...
      String response) {
    try {
      appSessionTxnDao.auditTransaction(
          sessionId,
          operationId,
          userId,
          txnStartTime,
