...
public class DBEncryptionUtil {
  ...
  public static void main(String[] args) {
    if (args == null || args.length == 0) {
      ...
      System.out.println(
          "Note: Set Database password encryption salt key as env variable "
              + DB_PASSWORD_ENCRYPTION_SALT_KEY);
  ...
  private static BasicTextEncryptor getEncryptorInstance() {
    ...
    if (dbPasswordEncryptionSaltKey == null) {
      throw new RuntimeException(
          "Database password encryption salt key is not set. Set env variable "
              + DB_PASSWORD_ENCRYPTION_SALT_KEY);
    }
