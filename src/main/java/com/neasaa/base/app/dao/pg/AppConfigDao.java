...
public class AppConfigDao extends AbstractDao {
  ...
  private PreparedStatement buildInsertStatement(Connection aConection, AppConfig aAppConfig)
      throws SQLException {
    String sqlStatement =
        "INSERT INTO LKPCONFIG (CONFIGNAME, PARAMNAME, PARAMVALUE, ENABLE, LISTORDERSEQ, CREATEDBY, CREATEDDATE, LASTUPDATEDBY, LASTUPDATEDDATE) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
            ...
            new PreparedStatementCreator() {
              ...
              public PreparedStatement createPreparedStatement(Connection aConection)
                ...
                String deleteSqlQuery =
                    "DELETE FROM LKPCONFIG WHERE CONFIGNAME = ? and PARAMNAME = ?";
  ...
  public PreparedStatement buildUpdateStatement(Connection aConection, AppConfig aAppConfig)
      throws SQLException {
    String updateStatement =
        "UPDATE LKPCONFIG SET PARAMVALUE = ? , ENABLE = ? , LISTORDERSEQ = ? , CREATEDBY = ? , CREATEDDATE = ? , LASTUPDATEDBY = ? , LASTUPDATEDDATE = ?  where CONFIGNAME = ? and PARAMNAME = ?";
  ...
  public AppConfig fetchAppConfig(AppConfig aAppConfig) throws SQLException {
    String selectQuery =
        "select  CONFIGNAME , PARAMNAME , PARAMVALUE , ENABLE , LISTORDERSEQ , CREATEDBY , CREATEDDATE , LASTUPDATEDBY , LASTUPDATEDDATE  from LKPCONFIG where CONFIGNAME = ?  and PARAMNAME = ? ";
    return getJdbcTemplate()
        .queryForObject(
            selectQuery,
            new AppConfigRowMapper(),
            aAppConfig.getConfigName(),
            aAppConfig.getParamName());
  }
}
