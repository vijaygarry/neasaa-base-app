...
public class OtpVerificationHistoryDao extends AbstractDao {
	...
	private PreparedStatement buildInsertStatement(Connection aConection, OtpVerificationHistory aOtpVerificationHistory) throws SQLException {
		String sqlStatement = "INSERT INTO OTPVERIFICATIONHISTORY (EMAILID, OTPTYPE, HASHOTPCODE, REQUESTID, STATUS, VERIFIEDAT, EXPIRYDATE, ATTEMPTS, LASTATTEMPTDATE, CREATEDDATE, LASTUPDATEDDATE) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
	...
	public int insertOtpVerificationHistory(OtpVerificationHistory aOtpVerificationHistory) throws SQLException {
	...
	public int deleteOtpVerificationHistory(OtpVerificationHistory aOtpVerificationHistory) throws SQLException {
		return getJdbcTemplate().update(new PreparedStatementCreator() {
			@Override
	...
	public PreparedStatement buildUpdateStatement(Connection aConection, OtpVerificationHistory aOtpVerificationHistory) throws SQLException {
		String updateStatement = "UPDATE OTPVERIFICATIONHISTORY SET EMAILID = ? , OTPTYPE = ? , HASHOTPCODE = ? , REQUESTID = ? , STATUS = ? , VERIFIEDAT = ? , EXPIRYDATE = ? , ATTEMPTS = ? , LASTATTEMPTDATE = ? , CREATEDDATE = ? , LASTUPDATEDDATE = ?  where SEQID = ?";
	...
	public int updateOtpVerificationHistory(OtpVerificationHistory aOtpVerificationHistory) throws SQLException {
		return getJdbcTemplate().update(new PreparedStatementCreator() {
			@Override
			public PreparedStatement createPreparedStatement(Connection aCon) throws SQLException {
	...
	public OtpVerificationHistory fetchOtpVerificationHistory(OtpVerificationHistory aOtpVerificationHistory) throws SQLException {
		String selectQuery = "select  SEQID , EMAILID , OTPTYPE , HASHOTPCODE , REQUESTID , STATUS , VERIFIEDAT , EXPIRYDATE , ATTEMPTS , LASTATTEMPTDATE , CREATEDDATE , LASTUPDATEDDATE  from OTPVERIFICATIONHISTORY where SEQID = ? ";
		return getJdbcTemplate().queryForObject(selectQuery, new OtpVerificationHistoryRowMapper(), aOtpVerificationHistory.getSeqId());
