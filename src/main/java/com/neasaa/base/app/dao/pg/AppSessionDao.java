...
public class AppSessionDao extends AbstractDao {
  ...
  private static final String GET_SESSION_BYID =
      "SELECT  SESSIONID , USERID , CHANNELID , ACTIVE , "
          + "AUTHENTICATED , SESSIONCREATIONTIME , LOGOUTTIME , LASTACCESSTIME , EXITCODE , APPHOSTNAME , "
          + "CLIENTIPADDRESS , USER_AGENT  from "
          + BASE_SCHEMA_NAME
          + "TXTSESSION where SESSIONID = ? ";

  private static final String LOGOUT_SESSION_QUERY =
      " update "
          + BASE_SCHEMA_NAME
          + "TXTSESSION "
  ...
  private static final String UPDATE_LAST_ACCESS_TIME_QUERY =
      "update "
          + BASE_SCHEMA_NAME
          + "TXTSESSION "
  ...
  public boolean logoutSession(long sessionId, SessionExitCode aSessionExitCode) {
    ...
    int rowsUpdated =
        getJdbcTemplate()
            .update(
                LOGOUT_SESSION_QUERY,
                aSessionExitCode.getExitCode(),
                currDate,
                currDate,
                sessionId);
    if (rowsUpdated > 0) {
  ...
  private PreparedStatement buildInsertStatement(Connection aConection, AppSession aAppSession)
      throws SQLException {
    String sqlStatement =
        "INSERT INTO "
            + BASE_SCHEMA_NAME
            + "TXTSESSION (USERID, CHANNELID, ACTIVE, AUTHENTICATED, SESSIONCREATIONTIME, LOGOUTTIME, "
            + "LASTACCESSTIME, EXITCODE, APPHOSTNAME, CLIENTIPADDRESS, USER_AGENT) "
            + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

    PreparedStatement prepareStatement =
        aConection.prepareStatement(sqlStatement, new String[] {"sessionid"});
  ...
  public AppSession getSessionById(Long aSessionId) {
    List<AppSession> appSessions =
        getJdbcTemplate().query(GET_SESSION_BYID, new AppSessionRowMapper(), aSessionId);
  ...
  public PreparedStatement buildUpdateStatement(Connection aConection, AppSession aAppSession)
      throws SQLException {
    String updateStatement =
        "UPDATE TXTSESSION SET USERID = ? , CHANNELID = ? , ACTIVE = ? , AUTHENTICATED = ? , SESSIONCREATIONTIME = ? , LOGOUTTIME = ? , LASTACCESSTIME = ? , EXITCODE = ? , APPHOSTNAME = ? , CLIENTIPADDRESS = ? , USER_AGENT = ?  where SESSIONID = ?";
