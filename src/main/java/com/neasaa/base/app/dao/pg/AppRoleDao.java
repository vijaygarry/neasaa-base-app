...
public class AppRoleDao extends AbstractDao {
  private static final String SELECT_ALL_ACTIVE_ROLES_QUERY =
      "select  ROLEID , ROLEDESC , ENABLE , "
          + "CREATEDBY , CREATEDDATE , LASTUPDATEDBY , LASTUPDATEDDATE  "
          + "from "
          + BASE_SCHEMA_NAME
          + "LKPROLE where enable = true";
  private static final String SELECT_OPERATIONS_FOR_A_ROLE_QUERY =
      "select  OPERATIONID " + "from " + BASE_SCHEMA_NAME + "LKPROLEOPERATIONMAP where ROLEID = ?";

  public List<AppRole> getAllActiveRoles() {
    List<AppRole> roles =
        getJdbcTemplate().query(SELECT_ALL_ACTIVE_ROLES_QUERY, new AppRoleRowMapper());
    List<AppRole> rolesWithOperaions = new ArrayList<>();

    for (AppRole role : roles) {
      List<String> operationIds =
          getJdbcTemplate()
              .query(SELECT_OPERATIONS_FOR_A_ROLE_QUERY, new StringRowMapper(), role.getRoleId());
  ...
  private PreparedStatement buildInsertStatement(Connection aConection, AppRole aAppRole)
      throws SQLException {
    String sqlStatement =
        "INSERT INTO LKPROLE (ROLEID, ROLEDESC, ENABLE, CREATEDBY, CREATEDDATE, LASTUPDATEDBY, LASTUPDATEDDATE) VALUES (?, ?, ?, ?, ?, ?, ?)";
  ...
  public PreparedStatement buildUpdateStatement(Connection aConection, AppRole aAppRole)
      throws SQLException {
    String updateStatement =
        "UPDATE LKPROLE SET ROLEDESC = ? , ENABLE = ? , CREATEDBY = ? , CREATEDDATE = ? , LASTUPDATEDBY = ? , LASTUPDATEDDATE = ?  where ROLEID = ?";
  ...
  public AppRole fetchAppRole(AppRole aAppRole) throws SQLException {
    String selectQuery =
        "select  ROLEID , ROLEDESC , ENABLE , CREATEDBY , CREATEDDATE , LASTUPDATEDBY , LASTUPDATEDDATE  from LKPROLE where ROLEID = ? ";
    return getJdbcTemplate()
        .queryForObject(selectQuery, new AppRoleRowMapper(), aAppRole.getRoleId());
  }
}
