...
public class DataSourceConfig {
  ...
  public HikariDataSource dataSource() throws IOException {
    ...
    config.setMaximumPoolSize(
        Integer.parseInt(props.getProperty("app.datasource.pool.maximum-pool-size", "10")));
    config.setMinimumIdle(
        Integer.parseInt(props.getProperty("app.datasource.pool.minimum-idle", "3")));
    config.setIdleTimeout(
        Long.parseLong(props.getProperty("app.datasource.pool.idle-timeout", "30000")));
    config.setConnectionTimeout(
        Long.parseLong(props.getProperty("app.datasource.pool.connection-timeout", "20000")));
    config.setMaxLifetime(
        Long.parseLong(props.getProperty("app.datasource.pool.max-lifetime", "600000")));
    config.setAutoCommit(
        Boolean.parseBoolean(props.getProperty("app.datasource.pool.auto-commit", "false")));
    config.setConnectionTestQuery(
        props.getProperty("app.datasource.pool.connection-test-query", "SELECT 1"));
