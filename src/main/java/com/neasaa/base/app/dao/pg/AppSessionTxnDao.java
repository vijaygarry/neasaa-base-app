...
public class AppSessionTxnDao extends AbstractDao {
  ...
  public void auditTransaction(
      ...
      throws SQLException {
    insertAppSessionTxn(
        AppSessionTxn.builder()
            .sessionId(sessionId)
            .operationId(operationId)
            .userId(userId)
            .txnStartTime(txnStartTime)
            .txnLatencyMillis(txnLatency)
  ...
  private PreparedStatement buildInsertStatement(
      Connection aConection, AppSessionTxn aAppSessionTxn) throws SQLException {
    String sqlStatement =
        "INSERT INTO "
            + BASE_SCHEMA_NAME
            + "TXTSESSIONTXN (SESSIONID, OPERATIONID, USERID, TXNSTARTTIME, TXNLATENCYMILLIS, HTTPRESPONSECODE, REQUEST, RESPONSE) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
  ...
  public PreparedStatement buildUpdateStatement(Connection aConection, AppSessionTxn aAppSessionTxn)
      throws SQLException {
    String updateStatement =
        "UPDATE TXTSESSIONTXN SET SESSIONID = ? , OPERATIONID = ? , USERID = ? , TXNSTARTTIME = ? , TXNLATENCYMILLIS = ? , HTTPRESPONSECODE = ? , REQUEST = ? , RESPONSE = ?  where TXNID = ?";
  ...
  public AppSessionTxn fetchAppSessionTxn(AppSessionTxn aAppSessionTxn) throws SQLException {
    String selectQuery =
        "select  TXNID , SESSIONID , OPERATIONID , USERID , TXNSTARTTIME , TXNLATENCYMILLIS , HTTPRESPONSECODE , REQUEST , RESPONSE  from TXTSESSIONTXN where TXNID = ? ";
    return getJdbcTemplate()
        .queryForObject(selectQuery, new AppSessionTxnRowMapper(), aAppSessionTxn.getTxnId());
  }
}
