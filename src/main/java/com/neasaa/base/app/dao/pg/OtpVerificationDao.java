...
public class OtpVerificationDao extends AbstractDao {
  ...
  private static final String GET_OTP_INFORMATION_BY_EMAIL_AND_TYPE =
      "select  EMAILID , OTPTYPE , HASHOTPCODE , REQUESTID , STATUS , VERIFIEDAT , EXPIRYDATE , ATTEMPTS , LASTATTEMPTDATE , CREATEDDATE , LASTUPDATEDDATE  "
          + " from "
          + BASE_SCHEMA_NAME
  ...
  private static final String INSERT_INTO_HISTORY_FROM_MAIN =
      "INSERT INTO "
          + BASE_SCHEMA_NAME
          + "OTPVERIFICATIONHISTORY (EMAILID, OTPTYPE, HASHOTPCODE, REQUESTID, STATUS, VERIFIEDAT, EXPIRYDATE, ATTEMPTS, LASTATTEMPTDATE, CREATEDDATE, LASTUPDATEDDATE)"
          + " SELECT EMAILID, OTPTYPE, HASHOTPCODE, REQUESTID, ?, ?, EXPIRYDATE, ?, ?, CREATEDDATE, ? FROM "
          + BASE_SCHEMA_NAME
          + "OTPVERIFICATION "
          + " WHERE EMAILID = ?  and OTPTYPE = ?";

  private static final String DELETE_OTP =
      "DELETE FROM " + BASE_SCHEMA_NAME + "OTPVERIFICATION WHERE EMAILID = ?  and OTPTYPE = ?";

  public OtpVerification getOtpInformation(String emailId, OTPType otpType) {
    try {
      List<OtpVerification> otpVerificationList =
          getJdbcTemplate()
              .query(
                  GET_OTP_INFORMATION_BY_EMAIL_AND_TYPE,
                  new OtpVerificationRowMapper(),
                  emailId,
                  otpType.name());
  ...
  public void moveOtpToHistory(OtpVerification aOtpVerification) {
    try {
      ...
      log.info(
          "Inserting OTP to history for emailId: {} and otpType: {}",
          aOtpVerification.getEmailId(),
          aOtpVerification.getOtpType().name());
      int recordInserted =
          getJdbcTemplate()
              .update(
                  INSERT_INTO_HISTORY_FROM_MAIN,
                  aOtpVerification.getStatus().name(),
                  aOtpVerification.getVerifiedAt(),
                  aOtpVerification.getAttempts(),
                  aOtpVerification.getLastAttemptDate(),
                  currentTimestamp,
                  aOtpVerification.getEmailId(),
                  aOtpVerification.getOtpType().name());
      ...
      int recordsDeleted =
          getJdbcTemplate()
              .update(
                  DELETE_OTP, aOtpVerification.getEmailId(), aOtpVerification.getOtpType().name());
  ...
  private PreparedStatement buildInsertStatement(
      Connection aConection, OtpVerification aOtpVerification) throws SQLException {
    String sqlStatement =
        "INSERT INTO "
            + BASE_SCHEMA_NAME
            + "OTPVERIFICATION (EMAILID, OTPTYPE, HASHOTPCODE, REQUESTID, STATUS, VERIFIEDAT, EXPIRYDATE, ATTEMPTS, LASTATTEMPTDATE, CREATEDDATE, LASTUPDATEDDATE) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            ...
            new PreparedStatementCreator() {
              ...
              public PreparedStatement createPreparedStatement(Connection aConection)
                ...
                String deleteSqlQuery =
                    "DELETE FROM OTPVERIFICATION WHERE EMAILID = ? and OTPTYPE = ?";
  ...
  public PreparedStatement buildUpdateStatement(
    ...
    String updateStatement =
        "UPDATE OTPVERIFICATION SET HASHOTPCODE = ? , REQUESTID = ? , STATUS = ? , VERIFIEDAT = ? , EXPIRYDATE = ? , ATTEMPTS = ? , LASTATTEMPTDATE = ? , CREATEDDATE = ? , LASTUPDATEDDATE = ?  where EMAILID = ? and OTPTYPE = ?";
