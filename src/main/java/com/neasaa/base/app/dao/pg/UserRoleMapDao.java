...
public class UserRoleMapDao extends AbstractDao {
  ...
  private static final String SELECT_USER_ROLES_BY_USERID =
      "select  ROLEID " + "from " + BASE_SCHEMA_NAME + "MSTUSERROLEMAP where USERID = ?";
  ...
  private PreparedStatement buildInsertStatement(Connection aConection, UserRoleMap aUserRoleMap)
      throws SQLException {
    String sqlStatement =
        "INSERT INTO "
            + BASE_SCHEMA_NAME
            + "MSTUSERROLEMAP (USERID, ROLEID, CREATEDBY, CREATEDDATE, LASTUPDATEDBY, LASTUPDATEDDATE) VALUES (?, ?, ?, ?, ?, ?)";
            ...
            new PreparedStatementCreator() {
              ...
              public PreparedStatement createPreparedStatement(Connection aConection)
                ...
                String deleteSqlQuery =
                    "DELETE FROM MSTUSERROLEMAP WHERE USERID = ? and ROLEID = ?";
  ...
  public PreparedStatement buildUpdateStatement(Connection aConection, UserRoleMap aUserRoleMap)
      throws SQLException {
    String updateStatement =
        "UPDATE MSTUSERROLEMAP SET CREATEDBY = ? , CREATEDDATE = ? , LASTUPDATEDBY = ? , LASTUPDATEDDATE = ?  where USERID = ? and ROLEID = ?";
  ...
  public UserRoleMap fetchUserRoleMap(UserRoleMap aUserRoleMap) throws SQLException {
    String selectQuery =
        "select  USERID , ROLEID , CREATEDBY , CREATEDDATE , LASTUPDATEDBY , LASTUPDATEDDATE  from MSTUSERROLEMAP where USERID = ?  and ROLEID = ? ";
    return getJdbcTemplate()
        .queryForObject(
            selectQuery,
            new UserRoleMapRowMapper(),
            aUserRoleMap.getUserId(),
            aUserRoleMap.getRoleId());
  }
}
